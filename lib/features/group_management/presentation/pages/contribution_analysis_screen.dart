...
class ContributionAnalysisScreen extends ConsumerWidget {
    ...
    return _buildCardWrapper(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          if (stats.isEmpty)
            const Text('Chưa có dữ liệu GitHub')
          else
            ...stats.map((s) => _buildProgressBar(s)).toList(),
        ],
      ),
    );
  }
    ...
    return _buildCardWrapper(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          if (stats.isEmpty)
            const Text('Chưa có dữ liệu Google Docs')
          else
            ...stats.map((s) => _buildProgressBar(s)).toList(),
        ],
      ),
  ...
  Widget _buildProgressBar(dynamic stat) {
    final percentage = ((stat['percentage'] as num?)?.toDouble() ?? 0.0) / 100.0;
